...
export class FabButton {
    ...
    create(element: HTMLElement) {
        ...
        if (this.mElement) {
            ...
            this.mHammer.on("panend", (event) => {
                this.onPanEndBDP(event);
            });

        }
        // this.setTransform();
        this.setTransformBdp();
    ...
    setTransformBdp() {
        ...
        if (this.mContent) {
            if (this.mShowContent) {
                this.mContent.style.transform = "translate3d(" + (this.mPosition.current.x + this.mElementSize.width / 2 - this.mContent.clientWidth / 2) + "px," + (this.mPosition.current.y + this.mElementSize.height) + "px,0px)";
            }
        }
    }
    ...
    setRelativeHTMLContent(element: HTMLElement) {
        this.mContent = element;
    }

    setElementSize(element: HTMLElement) {
        this.mElementSize.set(element.clientWidth, element.clientHeight);
    }
    ...
    onPanEndBDP(event) {
        ...
        this.mPanStart.y = this.mPosition.current.y;
        this.mDirty = true;
        if (this.mEventListener) this.mEventListener(GestureEvent.PAN_END, { position: this.mPosition });
        this.checkOutOfBox();
    }
